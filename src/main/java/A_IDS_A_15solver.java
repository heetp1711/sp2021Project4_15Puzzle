...
public A_IDS_A_15solver(int[] puzzle, int h){
		Node startState = new Node(puzzle);		//node contains the start state of puzzle
        ...
        if (h == 1) {
		    solution = A_Star(startState, "heuristicOne");	//A* search with heuristic 1 (misplaced tiles)
        } else if (h == 2) {
            solution = A_Star(startState, "heuristicTwo");
        }
